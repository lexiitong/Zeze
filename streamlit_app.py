...
def load_data_from_s3(bucket_name, file_key):
    if conn:
        try:
            df = conn.read(f"s3://zezeapp/Felix_cleaned_dataset160824.csv", input_format="csv", ttl=600)
            return df
        except Exception as e:
...
def main():
    ...
    if st.button("Predict Best Call Time"):
        ...
        df = load_data_from_s3('zezeapp', 's3://zezeapp/Felix_cleaned_dataset160824.csv')
